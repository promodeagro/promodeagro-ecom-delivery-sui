import { Box, Button, Container, Flashbar, Header, Icon, Modal, SpaceBetween } from '@cloudscape-design/components'
...
const Payment = () => {
    ...
    const [comleteOrderModalVisible, setComleteOrderModalVisible] = React.useState(false);
    ...
    const [items, setItems] = React.useState([
      ...
      // useEffect hook to start the timer when isRunning is true
      useEffect(() => {
        let timer;
  ...
  return (
   ...
   <div style={{marginTop:18 }}>
    <Container>
        <SpaceBetween direction='vertical' size='xxs'>
            <Box  textAlign='center' variant='h4'>Amount to be Collect</Box>
            <Box textAlign='center' variant='h3'> <span style={{color:"#037F0C"}}>₹ 2980/-</span></Box>
            <div>
   ...
   <div style={{ marginTop:18}} className='flex aic jcsb'>
    ...
    <Box fontSize='16px' variant='h4'>Qr Code</Box>
    <div className="payment_lines"></div>
   </div>

   <Box margin={'xs'} textAlign='center'>Generate Your QR Code Below</Box>

{/* qr code container */}
{isQRGenereted && (
<div className='flex jcc'>
<Box color='text-status-error' textAlign='center' variant='strong'>{timeLeft} Seconds left to scan the QR code</Box>
</div>
 )}
...
{!isQRGenereted && (
          <Button onClick={handleQr} variant='normal'>Generated QR Code</Button>
        )}
</span>
   </div>
...
<div className='upi_img'>
   ...
   <div className='flex aic jcsb'>
    <div className="payment_lines"></div>
    <Box margin={'m'} fontSize='16px' variant='h4'>OR</Box>
    <div className="payment_lines"></div>
   </div>
   
   <Button onClick={()=> setComleteOrderModalVisible(true)} variant='primary' fullWidth>Collect Cash</Button>
...
<div className='flex jcc'>
<img src={orderComplete} style={{width: "178px",height:"auto"}} alt="" />
</div>

<Box variant='h3' textAlign='center' color='text-status-success'>Collecting ₹2980/- Cash</Box>
...
}} variant='primary' fullWidth>Complete Order</Button>
<Button fullWidth onClick={()=> setComleteOrderModalVisible(false)} >Go Back</Button>
</SpaceBetween>
