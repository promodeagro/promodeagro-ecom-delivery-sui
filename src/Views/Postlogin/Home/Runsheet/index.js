...
const Runsheet = () => {
  ...
  useEffect(() => {
    const riderId = localStorage.getItem("id")?.replace(/['"]+/g, ""); // Fetch riderId from localStorage
    if (riderId && runsheetId && !hasFetchedRunsheetDetail) {
      dispatch(fetchRunsheetDetail({ riderId, runsheetId }));
      setHasFetchedRunsheetDetail(true); // Mark as fetched
    }
  ...
  const handleSelectChange = (e) => {
    ...
    if (selectedRunsheet?.value) {
      ...
      setHasFetchedRunsheetDetail(false); // Reset fetch flag for new runsheet
    }
  };
